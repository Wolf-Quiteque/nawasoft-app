// Relative import (not '@/lib/...') so `node --test` can load this file directly.
import { sellableSeatCount } from './seats.js';
...
/**
 * One run per bus: the one under way or next. Buses can run more than once
 * a day, and a plain Map keyed by bus would let a later run silently
 * overwrite an earlier one (that bug showed a full bus as 9/52).
 */
export function currentRunByBus(runs, now = Date.now()) {
  const grouped = new Map();
  for (const run of runs) {
    if (!grouped.has(run.bus_id)) grouped.set(run.bus_id, []);
    grouped.get(run.bus_id).push(run);
  }
  const result = new Map();
  for (const [busId, busRuns] of grouped) result.set(busId, pickCurrentRun(busRuns, now));
  return result;
}
