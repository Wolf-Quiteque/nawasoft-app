...
/**
 * Counter sales settle on the spot, so both methods land as `paid`. A
 * `referencia` ticket would sit unpaid and its download page would refuse to
 * render, which defeats the point of issuing one instantly.
 */
export const INSTANT_PAYMENT_METHODS = ['cash', 'tpa'];
...
export function ticketDownloadUrl(transactionId) {
  const base = (process.env.NEXT_PUBLIC_WEBSITE_BASE_URL || 'https://www.nawabus.ao').replace(/\/+$/, '');
  return `${base}/bilhetes/${transactionId}`;
}
...
export async function issueInstantTickets(supabase, input, staffUserId) {
  ...
  if (!INSTANT_PAYMENT_METHODS.includes(payment_method)) {
    fail(400, 'Método de pagamento inválido. Use Dinheiro ou TPA.');
  }
  ...
  return {
    transaction_id: transactionId,
    download_url: ticketDownloadUrl(transactionId),
    count: issued.length,
