...
export default function RoutesView({ initialData = null }) {
  const { data, loading, error } = useApi('/api/routes', { initialData });
          ...
          {data.provinces.map((group, gi) => {
            ...
            return (
              <Card key={group.province} className="overflow-hidden">
                ...
                {/* 0fr → 1fr is the CSS-only way to animate to auto height;
                    it replaces framer-motion's AnimatePresence here. */}
                <div
                  className={cn(
